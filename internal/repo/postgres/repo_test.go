...
// TestPostgres_Smoke exercises postgres-driver-specific behaviour
// (case-insensitive email lookup, pgerror→service.ErrAlreadyExists
// translation, OAuth-identity composite uniqueness via the Postgres
// unique index) against a live Postgres pointed to by
// GATEWAY_TEST_POSTGRES_DSN. Skips when the env var is unset so the
// default unit-test job in CI passes without a backing service.
//
// Driver-agnostic Repository semantics live in TestConformance.
...
// against a live Postgres. CI's `Conformance / postgres` matrix entry
// invokes this test; locally `make conformance-all` does the same
// after `make services-up`.
//
// Each subtest gets a fresh tenant id so state never leaks between
// subtests, and Close() is registered on every constructed pool.
...
func seedProject(ctx context.Context, t *testing.T, repo *pgRepository, projectID string) {
	t.Helper()
	_, err := NewProjectStore(repo, true, nil).createProject(ctx, &Project{
		ID:             projectID,
...
// from the env-driven TestPostgres_Smoke and the testcontainers-driven
// TestPostgres_Container (see repo_dockertest_test.go). The
// driver-agnostic Repository semantics live in TestConformance — this
...
func runRepositorySmoke(t *testing.T, dsn, projectID string) {
	...
	// pgerror → service.ErrAlreadyExists translation lives in
	// wrapPgErr; assert the wiring carries through on duplicate
	// inserts.
