...
func (r *sqliteRepository) CreateSSOSession(ctx context.Context, s *service.SSOSessionRecord) (string, error) {
	if s == nil {
		return "", errors.New("sqlite: CreateSSOSession: nil record")
	}
	id := s.NodeID
	if id == "" {
		id = newID()
	...
	const q = `
		INSERT INTO sso_sessions (
			id, project_id, token_hash, user_id, login_method, expires_at_ms, created_at_ms
		) VALUES ($1, $2, $3, $4, $5, $6, $7)`
	_, err := r.db.Exec(ctx, q, id, r.projectID, s.TokenHash, s.UserID, s.LoginMethod, s.ExpiresAt, s.CreatedAt)
	if err != nil {
...
// its own clock.
func (r *sqliteRepository) GetSSOSessionByHash(ctx context.Context, tokenHash string) (*service.SSOSessionRecord, error) {
	if tokenHash == "" {
		return nil, nil
	}
	const q = `
	...
	err := r.db.QueryRow(ctx, q, r.projectID, tokenHash).Scan(
		&s.NodeID, &s.TokenHash, &s.UserID, &s.LoginMethod, &s.ExpiresAt, &s.CreatedAt,
	)
	if noRows(err) {
		return nil, nil
	}
	if err != nil {
		return nil, wrapErr("GetSSOSessionByHash", err)
	}
	return &s, nil
}
...
func (r *sqliteRepository) RevokeSSOSessionsForUser(ctx context.Context, userID string) error {
	const q = `
