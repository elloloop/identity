...
func (r *sqliteRepository) FindQrLoginSession(ctx context.Context, sessionID string) (*service.QrLoginSessionRecord, error) {
	if sessionID == "" {
		return nil, nil
	}
	const q = `
	...
	err := r.db.QueryRow(ctx, q, r.projectID, sessionID).Scan(
	...
	if err != nil {
		return nil, wrapErr("FindQrLoginSession", err)
	}
	return &s, nil
}

func (r *sqliteRepository) CreateQrLoginSession(ctx context.Context, s *service.QrLoginSessionRecord) (string, error) {
	if s == nil {
		return "", errors.New("sqlite: CreateQrLoginSession: nil record")
	}
	id := s.NodeID
	if id == "" {
		id = newID()
	...
	if err != nil {
		return "", wrapErr("CreateQrLoginSession", err)
	}
	s.NodeID = id
	return id, nil
}
...
func (r *sqliteRepository) UpdateQrLoginSession(ctx context.Context, nodeID string, fields map[string]any) error {
	if nodeID == "" {
		return errors.New("sqlite: UpdateQrLoginSession: missing node id")
	}
	if len(fields) == 0 {
		return nil
	}
	...
	if _, err := r.db.Exec(ctx, q, args...); err != nil {
		return wrapErr("UpdateQrLoginSession", err)
	}
	return nil
}

func (r *sqliteRepository) ConsumeQrLoginSession(ctx context.Context, nodeID string, atMs int64) error {
	if nodeID == "" {
	...
	tag, err := r.db.Exec(ctx, q, r.projectID, nodeID, atMs)
	if err != nil {
		return wrapErr("ConsumeQrLoginSession", err)
	}
