...
func (r *sqliteRepository) FindInvitationByHash(ctx context.Context, tokenHash string) (*service.InvitationRecord, error) {
	if tokenHash == "" {
		return nil, nil
	}
	const q = `
	...
	if err != nil {
		return nil, wrapErr("FindInvitationByHash", err)
	}
	return &inv, nil
}
...
func (r *sqliteRepository) UpdateInvitation(ctx context.Context, nodeID string, fields map[string]any) error {
	if nodeID == "" {
		return errors.New("sqlite: UpdateInvitation: missing node id")
	}
	if len(fields) == 0 {
		return nil
	}
	...
	if _, err := r.db.Exec(ctx, q, args...); err != nil {
		return wrapErr("UpdateInvitation", err)
	}
	return nil
}
