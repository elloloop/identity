...
| `config_json` | JSONB | N | Default `'{}'`. Per-project settings, decoded by `service.ParseProjectConfig`. Currently: `cors.allowed_origins` (array of bare scheme+host origins or one-label `https://*.parent` wildcard patterns, layered on the global `GATEWAY_ALLOWED_ORIGINS` floor). Reserved for enabled login methods, OAuth providers, email templates, TTLs. |
