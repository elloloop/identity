...
func fetchOIDCDiscovery(ctx context.Context, client *http.Client, discoveryURL string) (*oidcDiscoveryDocument, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: build discovery request: %w", ErrCodeExchangeFailed, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("%w: read discovery body: %w", ErrCodeExchangeFailed, err)
	}
	...
	if err := json.Unmarshal(body, &doc); err != nil {
		return nil, fmt.Errorf("%w: parse discovery document: %w", ErrCodeExchangeFailed, err)
	}
...
func fetchOIDCUserInfo(ctx context.Context, client *http.Client, userinfoURL, accessToken string) (*oidcUserInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: build userinfo request: %w", ErrIdentityVerification, err)
	}
	...
	body, err := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
	if err != nil {
		return nil, fmt.Errorf("%w: read userinfo body: %w", ErrIdentityVerification, err)
	}
	if resp.StatusCode != http.StatusOK {
	...
	if err := json.Unmarshal(body, &info); err != nil {
		return nil, fmt.Errorf("%w: parse userinfo: %w", ErrIdentityVerification, err)
	}
	return &info, nil
}
