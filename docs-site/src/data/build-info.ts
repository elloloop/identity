...
export function getEditUrl(pageUrlPath: string): string {
  const astroPath = pageUrlToAstroPath(pageUrlPath);
  if (!astroPath) {
    return "https://github.com/elloloop/identity/tree/main/docs-site";
  }
  const rel = path.relative(REPO_ROOT, astroPath);
  return `https://github.com/elloloop/identity/edit/main/${rel}`;
}

function pageUrlToAstroPath(pageUrlPath: string): string | null {
  // pageUrlPath looks like "/identity/docs/quickstart/" or
  // "/identity/" for the root. Strip the base and the trailing slash.
  const base = "/identity";
...
export async function getGithubStars(): Promise<string | null> {
  ...
  try {
    ...
    const res = await fetch(
      "https://api.github.com/repos/elloloop/identity",
      {
