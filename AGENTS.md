...
- `ghcr.io/elloloop/tenant-shard-db:2.5.0` — never `:2`, never `:latest`
  (and the server image must track the SDK pinned in `go.mod`; CI's
  `entdb SDK / server-image version guard` enforces it — see #180)
